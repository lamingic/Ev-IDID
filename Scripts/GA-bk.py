...
class MA(object):
    def __init__(self, solver, filepath, result, dstr):
        self.domain = dstr
        self.solver = solver
        self.type = 'MA'
    ...
    def main(self):
        ...
        if os.path.isfile(self.filename):
            ...
            self.result['policy_tree'] = PolicyTree(
                self.domain + '-The Policy Tree of ' + self.type + ' for ' + self.solver.get(
            ...
            self.result['policy_tree'].save_policytree('./Results/')
            # self.dbn.result = self.result


class GA(object):
    def __init__(self, solver, filepath, result, dstr, type=None):
        if type is None:
            self.type = 'GA'
        else:
            self.type = type
    ...
    def evaluate_reward(self):
        ...
        for gi in range(0, len(self.pop)):
            ...
            mat = self.genome_to_mat(genome)
            pathes = self.mat_to_pathes(mat)
            policy_dict = dict()
    ...
    def create_genomes(self):
        ...
        for key in self.result['policy_dict'].keys():
            ...
            mat = self.pathes_to_mat(pathes)
            genome = self.mat_to_genome(mat)
    ...
    def plot_fits(self):
        ...
        plt.show()
        if self.parameters.values.get('group_criterion_method') is None:
            fig.savefig('./Results/' + self.domain + '-The FCL of ' + self.type + ' for ' + self.solver.get(
...
class PGA(object):
    ...
    def ga_main(self):
        for g in range(0, self.parameters.values.get('group_size')):
            ...
            self.ga_alg = GA(self.solver, self.filepath, self.result, self.domain, self.type)
            self.ga_alg.ga_main()
...
class GGA(GA):
    def __init__(self, solver, filepath, result, dstr):
        ...
        self.result = result
        self.pnames = Name()
        super(GGA, self).__init__(solver, filepath, result, self.domain, self.type)
